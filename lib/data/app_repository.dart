...
class AppRepository {
  ...
  Future<void> _seedMockDataIfNeeded(Database db) async {
    final counts = await Future.wait([
      db.rawQuery('SELECT COUNT(*) AS count FROM shops'),
      db.rawQuery('SELECT COUNT(*) AS count FROM products'),
      db.rawQuery('SELECT COUNT(*) AS count FROM sales'),
      db.rawQuery('SELECT COUNT(*) AS count FROM collections'),
    ]);

    final hasExistingData = counts.any((rows) {
    ...
    await db.transaction((txn) async {
      final shops = <Map<String, Object?>>[
        ...
        {
          'name': 'Wellness Care Point',
          'owner_contact': 'S. Fernando',
          'area': 'Maharagama',
          'phone': '0771234502',
          'credit_limit': 95000.0,
          'balance': 0.0,
          'is_active': 1,
          'created_at': threeDaysAgo.toIso8601String(),
          'updated_at': now.toIso8601String(),
        ...
        {
          'name': 'Family Choice Stores',
          'owner_contact': 'K. Silva',
          'area': 'Nugegoda',
          'phone': '0771234504',
        ...
        },
        {
          ...
          'owner_contact': 'Ruwan Jayasekara',
          'area': 'Moratuwa',
          'phone': '0771234505',
          'credit_limit': 175000.0,
          'balance': 0.0,
          'is_active': 1,
      ...
      final shopIds = <String, int>{};
      for (final shop in shops) {
        final id = await txn.insert('shops', shop);
        shopIds[shop['name'] as String] = id;
      }

      final products = <Map<String, Object?>>[
        {
          'name': 'Bio Care Cough Relief Syrup',
          'sku': 'BC-CRS-100',
          'unit_price': 480.0,
          'description': 'Herbal cough relief syrup 100ml',
        ...
        {
          'name': 'Bio Care Vitamin C Tablets',
          'sku': 'BC-VTC-030',
          'unit_price': 950.0,
          'description': 'Vitamin C immune support tablets pack',
        ...
        {
          'name': 'Bio Care Pain Relief Balm',
          'sku': 'BC-PRB-050',
          'unit_price': 390.0,
          'description': 'Topical balm for quick relief',
        ...
        {
          'name': 'Bio Care Multivitamin Syrup',
          'sku': 'BC-MVS-200',
          'unit_price': 760.0,
          'description': 'Daily multivitamin syrup 200ml',
        ...
        {
          'name': 'Bio Care Energy Plus Capsules',
          'sku': 'BC-EPC-020',
          'unit_price': 1120.0,
          'description': 'Energy support capsules pack of 20',
        ...
        {
          'name': 'Bio Care Digestive Tea',
          'sku': 'BC-DGT-025',
          'unit_price': 640.0,
          'description': 'Digestive wellness tea sachets',
      ...
      for (final product in products) {
        ...
        productIds[product['name'] as String] = id;
      }
      ...
      final sales = <({
        ...
        (
          ...
          items: [
            {
              'product_name': 'Bio Care Cough Relief Syrup',
              'quantity': 5,
              'unit_price': 480.0,
              'line_total': 2400.0,
            },
            {
              'product_name': 'Bio Care Pain Relief Balm',
              'quantity': 5,
              'unit_price': 390.0,
              'line_total': 1950.0,
            },
        ...
        (
          shopName: 'Wellness Care Point',
          paymentType: 'Credit',
          note: 'Requested 7-day settlement',
          subtotal: 6380.0,
          discount: 380.0,
          total: 6000.0,
          createdAt: today.add(const Duration(hours: 2, minutes: 15)),
          items: [
            {
              'product_name': 'Bio Care Vitamin C Tablets',
              'quantity': 4,
              'unit_price': 950.0,
              'line_total': 3800.0,
            },
            {
              'product_name': 'Bio Care Digestive Tea',
              'quantity': 4,
              'unit_price': 640.0,
              'line_total': 2560.0,
            },
        ...
        (
          shopName: 'City Med House',
          paymentType: 'Credit',
          note: 'Restock after weekend demand',
          subtotal: 8300.0,
          discount: 300.0,
          total: 8000.0,
          ...
          items: [
            {
              'product_name': 'Bio Care Multivitamin Syrup',
              'quantity': 5,
              'unit_price': 760.0,
              'line_total': 3800.0,
            },
            {
              'product_name': 'Bio Care Energy Plus Capsules',
              'quantity': 4,
              'unit_price': 1120.0,
              'line_total': 4480.0,
            },
        ...
        (
          shopName: 'CarePlus Distributors',
          paymentType: 'Cash',
          note: 'Bulk secondary shelf refill',
          subtotal: 7040.0,
          discount: 240.0,
          total: 6800.0,
          createdAt: twoDaysAgo.add(const Duration(hours: 3)),
          items: [
            {
              'product_name': 'Bio Care Digestive Tea',
              'quantity': 6,
      ...
      for (final sale in sales) {
        ...
        for (final item in sale.items) {
          await txn.insert('sale_items', {
            'sale_id': saleId,
            'product_id': productIds[item['product_name']],
      ...
      final collections = <Map<String, Object?>>[
        {
          'shop_name': 'Wellness Care Point',
          'amount': 2500.0,
          'payment_method': 'Bank',
          'reference_note': 'Partial bank transfer',
          'status': 'active',
          'void_reason': null,
          'created_at': today.add(const Duration(hours: 5)),
        },
        {
          'shop_name': 'City Med House',
          'amount': 3000.0,
          'payment_method': 'Cheque',
          'reference_note': 'Weekly cheque collection',
          'status': 'active',
          'void_reason': null,
          'created_at': yesterday.add(const Duration(hours: 4)),
        },
        {
          'shop_name': 'Wellness Care Point',
          'amount': 1200.0,
          'payment_method': 'Cash',
          'reference_note': 'Counter cash settlement',
          'status': 'active',
          'void_reason': null,
          'created_at': threeDaysAgo.add(const Duration(hours: 6)),
        },
      ];

      for (final collection in collections) {
        await txn.insert('collections', {
          'shop_id': shopIds[collection['shop_name']],
          'shop_name': collection['shop_name'],
          'amount': collection['amount'],
          'payment_method': collection['payment_method'],
          'reference_note': collection['reference_note'],
          'status': collection['status'],
          'void_reason': collection['void_reason'],
          'created_at': (collection['created_at'] as DateTime).toIso8601String(),
        });
      }
      ...
      await txn.rawUpdate('''
        ...
        SET balance = COALESCE((
          SELECT SUM(CASE WHEN LOWER(payment_type) = 'credit' AND status = 'active' THEN total ELSE 0 END)
          FROM sales
          WHERE sales.shop_id = shops.id
        ), 0) - COALESCE((
          SELECT SUM(CASE WHEN status = 'active' THEN amount ELSE 0 END)
          FROM collections
          WHERE collections.shop_id = shops.id
        ), 0),
        updated_at = ?
      ''', [now.toIso8601String()]);
    });
